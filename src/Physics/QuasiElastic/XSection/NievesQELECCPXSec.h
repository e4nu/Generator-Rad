...
\class    genie::NievesQELECCPXSec

\brief    Computes neutrino-nucleon(nucleus) QELECC differential cross section
...
#ifndef _NIEVES_QELECC_CROSS_SECTION_H_
#define _NIEVES_QELECC_CROSS_SECTION_H_
...
typedef enum EEQELRmax {
  // Use the same maximum radius as VertexGenerator (3*R0*A^(1/3))
  kMatchEVertexGeneratorRmax,

  // Use the method for calculting Rmax from Nieves' Fortran code
  kMatchENieves
} Nieves_ECoulomb_Rmax_t;

class QELFormFactorsModelI;
class XSecIntegratorI;

class NievesQELECCPXSec : public XSecAlgorithmI {

public:
  NievesQELECCPXSec();
  NievesQELECCPXSec(string config);
  virtual ~NievesQELECCPXSec();
...
private:
  ...
  Nieves_ECoulomb_Rmax_t fECoulombRmaxMode;
 ...
 namespace utils {
  namespace gsl   {
   namespace wrap   {
    ...
    class NievesQELEvcrIntegrand : public ROOT::Math::IBaseFunctionOneDim
    {
     public:
      NievesQELEvcrIntegrand(double Rcurr, int A, int Z);
      ~NievesQELEvcrIntegrand();
